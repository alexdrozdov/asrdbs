...
class Worddb(common.db.Db):
    def __init__(self, dbfilename, rw=False, no_classes=False):
        ...
        self.__wpi_shadow = WordPrimariesShadow(reuse_db=self)
    ...
    def get_word_primaries(self, word):
        return self.__wpi_shadow.get_object(word)
...
class WordInfoShadow(common.db.Db, common.shadow.Shadow):
    def __init__(self, reuse_db, lru_len=500000):
    ...
    def get_object_cb(self, objid):
        word = objid
        jblob = self.execute('SELECT blob FROM word_blobs WHERE (word_blobs.word=?);', (word,)).fetchall()
        if len(jblob) > 0 and len(jblob[0]) > 0:
            return json.loads(jblob[0][0])
        return None
...
class WordPrimariesShadow(common.db.Db, common.shadow.Shadow):
    def __init__(self, reuse_db, lru_len=1000000):
    ...
    def get_object_cb(self, objid):
        ...
        jblob = self.execute('SELECT blob FROM word_blobs WHERE (word_blobs.word=?);', (word,)).fetchall()
        if len(jblob) > 0 and len(jblob[0]) > 0:
            word_info = json.loads(jblob[0][0])
            res = []
            for wf in word_info:
                res.append(wf['primary']['word'])
            return res
        return None
